...
class carrier_attrs(basic_attrs, Protocol):
    carrier_dynamic_viscosity: float # Dynamic viscosity of the carrier gas (kg m-1 s-1)
    carrier_density: float # Density of the carrier gas (kg m-3)

class full_attrs(carrier_attrs, Protocol):
    reactant_diffusion_rate: float # Diffusion rate of the reactant (cm2 s-1)
    reactant_molec_velocity: float # Thermal molecular velocity of the reactant (cm s-1)

### Flow rate conversion functions ###
def sccm_to_ccm(obj: basic_attrs, FR: float) -> float:
        ...
        return (tools.STANDARD_PRESSURE_Pa / obj.P) * obj.T / tools.STANDARD_TEMPERATURE_K * FR

def ccm_to_sccm(obj: basic_attrs, FR: float) -> float:
    ...
    return (obj.P / tools.STANDARD_PRESSURE_Pa) * tools.STANDARD_TEMPERATURE_K / obj.T * FR

def sccm_to_velocity(obj: basic_attrs, FR: float, diameter: float) -> float:
...
def MR_to_molec(obj: basic_attrs, conc: float) -> float:
...
def molec_velocity(obj: basic_attrs, molar_mass: float) -> float:
    ...
    return 100 * np.sqrt(8 / np.pi * tools.UNIVERSAL_GAS_CONSTANT * obj.T / molar_mass * 1000)
...
def reynolds_number(obj: carrier_attrs, FR: float, diameter: float) -> float:
    ...
    return (obj.carrier_density / 100**3) * sccm_to_velocity(obj, FR, diameter
                                                             ) * diameter / (obj.carrier_dynamic_viscosity / 100)

def conductance(obj: carrier_attrs, diameter: float, length: float) -> float:
    ...
    return 32600 * diameter ** 4 / (obj.carrier_dynamic_viscosity * 1e7 * length) * obj.P / tools.P_CF['Torr']
    
def pressure_gradient(obj: basic_attrs, conductance: float, FR: float) -> float:
...
def buoyancy_parameters(obj: carrier_attrs, delta_T: float, distance: float, Re: float) -> float:
    ...
    Returns:
        float: Buoyancy parameter (>1 indicates the flow being driven by buoyancy).
    '''

    # Grashof Number - eq. 9.12 from Incropera, et al., 2007
    grashof_number = 9.81 / obj.T * delta_T * distance ** 3 / (obj.carrier_dynamic_viscosity / 100 / 
                                                               (obj.carrier_density / 100 ** 3)) ** 2
...
def mixing_time(obj: full_attrs, diameter: float) -> float:
    ...
    Args:
        obj (full_attrs): Object with full attributes (P in Pa, T in K, reactant_diffusion_rate in cm2 s-1,
                          carrier_dynamic_viscosity in kg m-1 s-1, carrier_density in kg m-3).
        diameter (float): Diameter of the cylinder (cm).
...
def N_eff_Shw(obj: full_attrs, length: float, FR: float) -> float:
    ...
    z_star = length * np.pi / 2 * obj.reactant_diffusion_rate / (sccm_to_ccm(obj, FR) / 60)

    return 3.6568 + 0.0978 / (z_star + 0.0154)

def Kn(mean_free_path: float, char_length: float) -> float:
...
def diffusion_limited_rate_constant(obj: full_attrs, N_eff_Shw: float, diameter: float) -> float:
...
def diffusion_limited_uptake_coefficient(obj: full_attrs, diameter: float, k_diff: float) -> float:
...
def correction_factor(N_eff_Shw: float, Kn: float, gamma: NDArray[np.float64] | float) -> NDArray[np.float64] | float:
...
def observed_loss_rate(
        obj: full_attrs, diameter: float, gamma_eff: NDArray[np.float64] | float) -> NDArray[np.float64] | float:
...
def cylinder_loss(obj: full_attrs, diameter: float, N_eff_Shw: float, Kn: float, gamma: NDArray[np.float64] | float, 
                time: float) -> NDArray[np.float64] | float:
    ...
    Returns:
        float: Penetration - fraction of initial reactant after passing through cylinder (unitless).
    '''
     
    return 1 - np.exp(-gamma / (1 + gamma * 3 / (2 * N_eff_Shw * Kn)) * obj.reactant_molec_velocity / diameter * time)
...
''' Citations:
    ...
    Incropera, F.P., DeWitt, D.P., Bergman, T.L., Lavine, A.S. (Eds.), 2007. Fundamentals of heat and mass transfer, 
    6. ed. ed. Wiley, Hoboken, NJ.
