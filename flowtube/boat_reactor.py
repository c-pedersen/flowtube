...
class BoatReactor:
    def __init__(
            self, FT_ID: float, FT_length: float, injector_ID: float, injector_OD: float, reactant_gas: str, 
            carrier_gas: str, reactant_MR: float, boat_width: float, boat_height: float, boat_length: float, 
            boat_wall_thickness: float) -> None:
        '''
        Handles calculations relevant to flow rate, flow diagnostics, transport, and uptake for a boat reactor (partial 
        cylinder (less than half) inside of a cylinder). Since there is no diffusion correction for the boat reactor 
        geometry, the assumption is that gas-phase diffusion is negligible. Assumes that the boat reactor is filled to 
        the brim with a liquid. The injector is assumed to be above the boat reactor at all times. All calculations are
        for over the boat.
        ...
        Args:
            ...
            reactant_gas (str): Molecular formula of reactant gas (supported Ar, He, Air, Br2, Cl2, HBr, HCl, HI, H2O, 
                I2, NO, N2, and O2).
        ...
        if reactant_gas not in diffusion_coef.sigmas.keys():
            raise ValueError(f"Unsupported reactant gas. Supported gases: {', '.join(diffusion_coef.sigmas.keys())}")
        if carrier_gas not in viscosity_density.a.keys():
            raise ValueError(f"Unsupported carrier gas. Supported gases: {', '.join(viscosity_density.a.keys())}")
        
        # Check physicality of insert dimensions
        if boat_length < 0 or boat_height < 0 or boat_width < 0 or boat_wall_thickness < 0:
    ...
    def initialize(self, reactant_FR: float, reactant_carrier_FR: float, carrier_FR: float, P: float, P_units: str, 
                   T: float, radial_delta_T: float = 1, axial_delta_T: float = 1, disp: bool = True) -> None:
        ...
        if P_units not in tools.P_CF.keys():
            raise ValueError(f"Unsupported pressure units. Supported units: {', '.join(tools.P_CF.keys())}")
        elif P < 0:
        ...
        self.carrier_flow(radial_delta_T=radial_delta_T, axial_delta_T=axial_delta_T, disp=disp)
        self.reactant_diffusion(disp=disp)

    def flows(self, reactant_FR: float, reactant_carrier_FR: float, carrier_FR: float, disp: bool = True) -> None:
        ...
        var_names: list[str] = []; var: list[float] = []; var_fmts: list[str] = []; units: list[str] = []

        # Flow Rate Setpoints
        var_names += ['Reactant Flow Rate']; var += [reactant_FR]; var_fmts += ['.2f']; units += ['sccm']
        var_names += ['Reactant Carrier Flow Rate']; var += [reactant_carrier_FR]; var_fmts += ['.1f']; 
        units += ['sccm']
        ...
        total_reactant_flow_velocity = flow_calc.sccm_to_velocity(self, total_reactant_FR, self.injector_ID)

        # Calculate the cross-sectional area of the boat reactor and flow tube
        boat_cross_section = tools.partial_cylinder_area(self.boat_height, self.boat_width)
        self.net_cross_section = tools.cross_sectional_area(self.FT_ID) - boat_cross_section
        ...
        min_carrier_FR = flow_calc.ccm_to_sccm(self, min_carrier_flow_velocity * (
            self.net_cross_section - tools.cross_sectional_area(self.injector_OD)) * 60)
        ...
        var_names += [f'Injector {self.reactant_gas} Concentration']; var += [injector_conc]; var_fmts += ['.3g'] 
        units += ['ppb']
        var_names += [f'Flow Tube {self.reactant_gas} Concentration']; var += [FT_conc]; var_fmts += ['.3g']; 
        units += ['ppb']
        var_names += [f'Flow Tube {self.reactant_gas} Concentration']; var += [FT_conc_molec]; var_fmts += ['.2e']
        units += ['molec. cm-3']
        
        # Total Flow Velocities
        self.flow_velocity = flow_calc.sccm_to_ccm(self, self.total_FR) / self.net_cross_section / 60
        var_names += ['Flow Tube Velocity']; var += [self.flow_velocity]; var_fmts += ['.3g']; units += ['cm s-1']
        
        # Residence Time over the boat
        self.residence_time = (self.boat_length - self.boat_wall_thickness * 2) / self.flow_velocity
        var_names += ['Boat Residence Time']; var += [self.residence_time]; var_fmts += ['.3g']; units += ['s']
    
        ### Display Values ###
        if disp == True:
            tools.table(var_names, var, var_fmts, units)
    
    def carrier_flow(self, radial_delta_T: float = 1, axial_delta_T: float = 1, disp: bool = True):
        ...
        self.carrier_dynamic_viscosity = viscosity_density.dynamic_viscosity(self, self.carrier_gas)
        var_names += ['Carrier Gas Dynamic Viscosity']; var += [self.carrier_dynamic_viscosity]; var_fmts += ['.2e'] 
        units += ['kg m-1 s-1']
        
        # Carrier Gas Density (kg m-3)
        self.carrier_density = viscosity_density.real_density(self, self.carrier_gas)
        var_names += ['Carrier Gas Density']; var += [self.carrier_density]; var_fmts += ['.3g']; units += ['kg m-3']
    ...
    def reactant_diffusion(self, disp: bool = True):
        ...
        var_names += ['Reactant Diffusion Rate']; var += [self.reactant_diffusion_rate]; var_fmts += ['.3g'] 
        units += ['cm2 s-1']
        ...
        self.reactant_molec_velocity = flow_calc.molec_velocity(
            self, float(mm.Formula(self.reactant_gas).mass)) # pyright: ignore[reportUnknownArgumentType, reportUnknownMemberType]
        
        # Reactant Mean Free Path (cm) - Fuchs and Sutugin, 1971
        reactant_mean_free_path = 3 * self.reactant_diffusion_rate / self.reactant_molec_velocity
        ...
        var_names += ['Peclet Number']; var += [Pe]; var_fmts += ['.4g']; units += ['unitless']
        if Pe < 10:
        ...
        # NOTE: the boat geometry is not considered and thus this value should be used as a limiting case
    ...
    def reactant_uptake(self, hypothetical_gamma: NDArray[np.float64] | float, gamma_wall: float = 5e-6, 
                        disp: bool = True) -> NDArray[np.float64] | float:
        ...
        Args:
            hypothetical_gamma (float or numpy.ndarray): Hypothetical uptake coefficient to calculate diffusion 
                correction factor.
            gamma_wall (float): Wall uptake coefficient (default: 5e-6 for halocarbon wax coating - 
                Ivanov et al., J. Mass Spectrom., 2021).
            disp (bool): Display calculated values.
        ...
        liquid_volume = tools.partial_cylinder_area(liquid_height, liquid_width) * liquid_length
        var_names += ['Boat Surface Area', 'Boat Volume']; var += [liquid_surface_area, liquid_volume]; 
        var_fmts += ['.1f', '.1f']; units += ['cm2', 'cm3']

        # Diffusion Correction - see flow_calc.py for details
        diff_corr = 1 - flow_calc.correction_factor(self.N_eff_Shw_FT, self.Kn_FT, hypothetical_gamma)
        if diff_corr > 0.05:
            warnings.warn('Diffusion correction is > 5%. Negligible diffusion may no longer be a valid assumption')
        var_names += ['Flow Tube Wall Diffusion Correction \n(must be small to neglect for boat reactor)']
        ...
        actual_SA_V_ratio = liquid_surface_area / (self.net_cross_section * liquid_length)
        geometric_correction = actual_SA_V_ratio / cylinder_SA_V_ratio
        var_names += ['Boat geometry correction factor']; var += [1 / geometric_correction]; var_fmts += ['.2f']
        units += ['unitless']
        
        # Corrected Loss Rate (s-1) - see flow_calc.py and Hanson and Ravishankara, 1993 for details
        k = flow_calc.observed_loss_rate(self, self.FT_ID, hypothetical_gamma) / geometric_correction
        var_names += ['Loss Rate']; var += [k]; var_fmts += ['.3g']; units += ['s-1']
        
        # Uptake to boat (fraction) - first order kinetics
        uptake = 1 - np.exp(-k * self.residence_time)
        var_names += ['Loss to Boat']; var += [uptake * 100]; var_fmts += ['.1f']; units += ['%']
        
        # Reactant Wall Loss (fraction) - calculated as if there is no boat, take as an upper limit
        reactant_wall_loss = flow_calc.cylinder_loss(
            self, self.FT_ID, self.N_eff_Shw_FT, self.Kn_FT, gamma_wall, self.residence_time)
        var_names += ['Estimated Wall Loss (upper limit)']; var += [reactant_wall_loss * 100]; var_fmts += ['.2g']; units += ['%']
...
''' Citations:
    ...
    Hanson, D.R., Ravishankara, A.R., 1993. Uptake of hydrochloric acid and hypochlorous acid onto sulfuric acid: 
    solubilities, diffusivities, and reaction. J. Phys. Chem. 97, 12309–12319. https://doi.org/10.1021/j100149a035
    
    Fuchs, N.A., Sutugin, A.G., 1971. HIGH-DISPERSED AEROSOLS, in: Hidy, G.M., Brock, J.R. (Eds.), Topics in Current 
    Aerosol Research, International Reviews in Aerosol Physics and Chemistry. Pergamon, p. 1. 
    https://doi.org/10.1016/B978-0-08-016674-2.50006-6

    Ivanov, A.V., Molina, M.J., Park, J., 2021. Experimental study on HCl uptake by MgCl2 and sea salt under humid 
    conditions. J Mass Spectrom 56, e4601. https://doi.org/10.1002/jms.4601

    Tang, M.J., Cox, R.A., Kalberer, M., 2014. Compilation and evaluation of gas phase diffusion coefficients of 
    reactive trace gases in the atmosphere: volume 1. Inorganic compounds. Atmos. Chem. Phys. 14, 9233–9247. 
